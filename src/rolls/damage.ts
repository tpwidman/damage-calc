...
import { addDamageAction } from '../turns/turn-history';
import { addToHistory } from '../menu/history';
import { sleep } from '../utils/animations';
import type { Config, AttackOptions, DamageResult, DieRoll } from '../types';
import { isSavageAttacksAvailable, useSavageAttacks, getSavageAttacksStatus } from '../features/savage-attacker';
import { isBrutalStrikeAvailable, useBrutalStrike, getBrutalStrikeStatus } from '../features/brutal-strike';
import { isHeroicInspirationAvailable, useHeroicInspiration, getHeroicInspirationStatus } from '../features/heroic-inspiration';

export async function rollDamageInteractive(config: Config): Promise<void> {
  console.clear();
  console.log(chalk.yellow.bold('💥 Standalone Damage Roll\n'));
  
  // Show current status
  const heroicStatus = getHeroicInspirationStatus(config);
  const brutalStatus = isBrutalStrikeAvailable(config)
  ...
  const result = await calculateDamage(config, options);
  await printResult(config, result, options);
  
  // Mark brutal strike as used if it was used
  if (options.brutal && isBrutalStrikeAvailable(config)) {
    useBrutalStrike(config);
  }
...
async function calculateDamage(config: Config, args: AttackOptions = {}): Promise<DamageResult> {
  ...
  const weaponResult = rollWeaponDamage(config, critical, savage && config.character.features.savage_attacks.enabled || false);
  ...
  if (brutal && isBrutalStrikeAvailable(config)) {
  ...
  if (isHeroicInspirationAvailable(config) && allRolls.length > 0) {
...
function rollWeaponDamage(config: Config, isCritical: boolean, useSavage: boolean): { damage: number; diceRolled: string[]; explanations: string[]; allRolls: DieRoll[] } {
...
async function printResult(config: Config, result: DamageResult, args: AttackOptions): Promise<void> {
  ...
  if (args.critical) {
    await animateCriticalHit(config);
...
async function animateCriticalHit(config: Config): Promise<void> {
  if (!config.settings.enable_crit_animations) {
    console.log('\n💥 CRITICAL HIT! 💥');
...
export async function calculateDamageFromAttack(
  ...
  if (!presetOptions.brutal && 
      isBrutalStrikeAvailable(config) && 
  ...
  const result = await calculateDamage(config, finalOptions);
  await printResult(config, result, finalOptions);
  
  // Mark brutal strike as used if it was used
  if (finalOptions.brutal && isBrutalStrikeAvailable(config)) {
    useBrutalStrike(config);
  }
